...
def deleteTile(tile_bag, letter):
    # find the count of the letter drawn
    for index, tile_tuple in enumerate(tile_counts_list):
        if (tile_tuple[0] == letter):
            tile_counts_list[index] = (letter, tile_tuple[1]  - 1)
...
def drawTiles(tile_bag):
    ...
    for x in range(num_tiles_drawn):
        drawn_tile_letter = random.choice(tile_bag)
        print("drawn ", drawn_tile_letter)
        deleteTile(tile_bag, drawn_tile_letter)
